...
/* return ResponseEntity.ok().header(HttpHeaders.SET_COOKIE, jwtCookie.toString())
  ...
  public ResponseEntity<?> registerAdmin(@Valid @RequestBody SignupRequest signUpRequest) {
    if (adminRepository.existsByPrenom_admin(signUpRequest.getPrenom_admin())) {
      return ResponseEntity.badRequest().body(new MessageResponse("Erreur: ce prenom existe deja"));
    }

    if (adminRepository.existsByNom_admin(signUpRequest.getNom_admin())) {
      return ResponseEntity.badRequest().body(new MessageResponse("Erreur: ce nom est deja utilisé !"));
    }

    if (adminRepository.existsByEmail_admin(signUpRequest.getEmail_admin())) {
